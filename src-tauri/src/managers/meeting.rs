...
pub struct MeetingManager {
    ...
    /// PRD generators for each meeting (by meeting_id)
    prd_generators: Arc<Mutex<HashMap<String, PRDGenerator>>>,
...
impl MeetingManager {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            prd_generators: Arc::new(Mutex::new(HashMap::new())),
    ...
    pub async fn start_meeting(&self, name: String) -> Result<String> {
        ...
        // Initialize PRD generator for this meeting
        {
            let mut generators = self.prd_generators.lock().await;
            let prd_gen = PRDGenerator::new(meeting_id.clone(), name.clone());
            generators.insert(meeting_id.clone(), prd_gen);
            log::info!("Initialized PRD generator for meeting: {}", meeting_id);
        }
    ...
    pub async fn end_meeting(&self, meeting_id: &str) -> Result<MeetingSummary> {
        ...
        if let Some(meeting) = meetings.get_mut(meeting_id) {
            ...
            // Generate final PRD if enabled
            let settings = settings::get_settings(&self.app_handle);
            if settings.enable_prd_generation {
                log::info!("Generating final PRD for meeting: {}", meeting_id);

                let prd_generators_clone = self.prd_generators.clone();
                let meeting_id_clone = meeting_id.to_string();
                let transcript_clone = meeting_snapshot.transcript_segments.clone();
                let app_handle_clone = self.app_handle.clone();

                tokio::spawn(async move {
                    let mut generators = prd_generators_clone.lock().await;
                    ...
                    // Clean up PRD generator from memory after final generation
                    generators.remove(&meeting_id_clone);
                });
            } else {
                // Clean up PRD generator even if generation is disabled
                let mut generators = self.prd_generators.lock().await;
                generators.remove(meeting_id);
            }
            ...
            if should_append_update {
                ...
                if let Some(project_path) = project_path {
                    if segments_snapshot.len() > last_sent_index {
                        ...
                        // Check if we should generate a PRD
                        if settings_now.enable_prd_generation {
                            let total_segments = segments_snapshot.len();
                            let prd_generators_clone = prd_generators.clone();
                            let meeting_id_prd = meeting_id.clone();
                            let app_handle_prd = app_handle.clone();

                            tokio::spawn(async move {
                                let should_generate = {
                                    let generators = prd_generators_clone.lock().await;
                                    if let Some(generator) = generators.get(&meeting_id_prd) {
                                        let versions = generator.get_all_versions();

                                        // Generate initial PRD if we have enough segments and no versions yet
                                        if versions.is_empty() {
                                            total_segments >= settings_now.prd_initial_min_segments
                                        } else {
                                ...
                                if should_generate {
                                    ...
                                    let mut generators = prd_generators_clone.lock().await;
                                    if let Some(generator) = generators.get_mut(&meeting_id_prd) {
