...
use super::{SystemAudioCapture, SystemAudioCaptureDevice, VirtualDeviceInfo};
use anyhow::Result;
            ...
            while let Ok(msg) = control_rx.recv() {
                match msg {
                    ControlMessage::Start { device_id, buffer } => {
                        ...
                        let in_accumulator: Arc<Mutex<Vec<f32>>> = Arc::new(Mutex::new(Vec::new()));
