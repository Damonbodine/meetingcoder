...
export const PreferWhisperForImports: React.FC<Props> = ({ descriptionMode = "inline" }) => {
  const settings = useSettingsStore((s) => s.settings);
  const updateSetting = useSettingsStore((s) => s.updateSetting);
  const value = settings?.prefer_whisper_for_imports ?? false;

  const onToggle = async (e: React.ChangeEvent<HTMLInputElement>) => {
    await updateSetting("prefer_whisper_for_imports", e.target.checked);
  };

  return (
    <div className="flex items-start justify-between py-2">
      <div>
        <div className="font-medium">Prefer Whisper for imports</div>
        {descriptionMode === "inline" && (
          <div className="text-sm text-neutral-500">
            When importing audio files or YouTube, use a downloaded Whisper model if available for higher accuracy.
          </div>
        )}
      </div>
