...
export const FastImportModeForImports: React.FC<Props> = ({ descriptionMode = "inline" }) => {
  const settings = useSettingsStore((s) => s.settings);
  const updateSetting = useSettingsStore((s) => s.updateSetting);
  const value = settings?.fast_import_mode_for_imports ?? true;

  const onToggle = async (e: React.ChangeEvent<HTMLInputElement>) => {
    await updateSetting("fast_import_mode_for_imports", e.target.checked);
  };

  return (
    <div className="flex items-start justify-between py-2">
      <div>
        <div className="font-medium">Fast import mode (greedy decode)</div>
        {descriptionMode === "inline" && (
          <div className="text-sm text-neutral-500">
            Use Whisper greedy decoding for imports to improve speed on long files.
          </div>
        )}
      </div>
